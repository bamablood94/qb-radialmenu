...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then Notify('Wrong Ped', Lang:t("info.wrong_ped"), 2500, 'info') return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		Notify('No Variants', Lang:t("info.no_variants"), 2500, 'info') return
	else
		if not LastEquipped[which] then
	...
	Notify('Already Wearing', Lang:t("info.already_wearing"), 2500, 'info') return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		Notify('Nothing Here', Lang:t("info.nothing_to_remove"), 2500, 'info') return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then Notify('Wrong Ped', Lang:t("info.wrong_ped"), 2500, 'info') return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		Notify('No Variants', Lang:t("info.no_variants"), 2500, 'info') return false
	end
end
...
function Notify(title, message, time, type) -- However you want your notifications to be shown, you can switch it up here.
